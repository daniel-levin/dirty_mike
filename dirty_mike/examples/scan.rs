...
fn benchmark_hashmap_lookup_int(
    data: &FxHashMap<i32, String>,
    target: i32,
) -> BasicHardwareCounters {
    let (result, counters) = BasicHardwareCounters::measure(|| data.get(&target).cloned()).unwrap();
...
fn benchmark_hashmap_lookup_string(
...
fn benchmark_binary_search_string(
    data: &[(String, String)],
    target: &str,
) -> BasicHardwareCounters {
    let (result, counters) = BasicHardwareCounters::measure(|| {
...
fn main() {
    ...
    let int_array: Vec<(i32, String)> = (0..ARRAY_SIZE as i32)
        .map(|i| (i, format!("value_{}", i * 2)))
        .collect();
    ...
    println!(
        "Array size: {}, Iterations per test: {}",
    ...
    for &pos in &test_positions {
        ...
        println!(
            "  Array Scan      - Cycles: {:.1}, Instructions: {:.1}, Branch Misses: {:.1}, Time: {:?}",
        ...
        println!(
            "  Binary Search   - Cycles: {:.1}, Instructions: {:.1}, Branch Misses: {:.1}, Time: {:?}",
        ...
        println!(
            "  FxHashMap Lookup - Cycles: {:.1}, Instructions: {:.1}, Branch Misses: {:.1}, Time: {:?}",
        ...
        for _ in 0..NUM_ITERATIONS {
            ...
            let binary_result =
                benchmark_binary_search_string(&string_sorted_array, &string_target);
