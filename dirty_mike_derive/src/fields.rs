            ...
            if attr.path().is_ident("hardware") {
                match &attr.meta {
                    Meta::List(list) => {
                        ...
                        return Ok(Self {
                            name: ident,
                            spec: ObservableEventSpec::Hardware(token_str),
                        });
                    }
                    _ => {
                        return Err(Error::new_spanned(
                ...
                match &attr.meta {
                    Meta::List(list) => {
                        ...
                        return Ok(Self {
                            name: ident,
                            spec: ObservableEventSpec::Raw(id),
                        });
                    }
                    _ => {
                        return Err(Error::new_spanned(
...
mod tests {
    ...
    fn test_mixed_extractions() {
        ...
        assert!(matches!(
            &fields[0],
            DesignatedField {
                spec: ObservableEventSpec::Raw(0xab),
                ..
            }
        ));
        ...
        assert!(matches!(
            &fields[1],
            DesignatedField {
                spec: ObservableEventSpec::Raw(0xcd),
                ..
            }
        ));
    }
